...
- ESP32-S3 桥接固件：通过 CC1101 收发 433 MHz 应援棒空口信号，支持 USB Serial、BLE 与 Wi-Fi HTTP 三种主机接口。
...
本项目的固件与协议仅针对以上实测型号设计。对其他应援棒是否有效、对频率与功率是否合规，均不做任何保证。
...
- 使用本项目的无线电发射前，请自行确认当地法规允许的频率、功率与使用场景。
